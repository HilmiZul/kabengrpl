      ...
      <div class="col-lg-12">
        <div class="card mb-5">
          <div class="card-header">
            <h4 class="title">
              {{ title }}
              <nuxt-link to="/pinjam/ruang/tambah" class="btn btn-danger btn-sm rounded-pill">Pinjam</nuxt-link>
...
async function loadMore() {
  ...
  let { data, error } = await client
    .from('inv_pinjam')
    .select(`
      id, room(namaRoom), peminjam, role, kategori,
...
function onDone(id) {
  if(confirm('Konfirmasi selesai penggunaan ruang praktik')) {
    itemID.value = id
    diKembalikan()
  }
}
...
const itemFiltered = computed(() => {
  return items.value.filter((i) => {
    return (
      i.room.namaRoom.toLowerCase().includes(keyword.value.toLowerCase()) ||
